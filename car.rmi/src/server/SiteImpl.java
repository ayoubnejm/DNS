...
public class SiteImpl extends UnicastRemoteObject implements SiteItf {
	...
	public void broadcast(final String data, final List<String> idBroadcasted)
	...
	public String getHierarchy() throws RemoteException {
		return this.getHierarchy(new ArrayList<String>());
	}

	 
	public String getHierarchy(List<String> idBroadcasted)
			throws RemoteException {
		// If already sent data => do nothing
		if (idBroadcasted.contains(this.id))
			return "";

		String hierarchy = this.id;

		hierarchy += " | ";

		if (this.parent != null)
			hierarchy += this.parent.getId();

		hierarchy += " | ";

		for (SiteItf child : this.children)
			hierarchy += child.getId() + " ; ";

		hierarchy += "\n";

		idBroadcasted.add(this.id);
		if (this.parent != null)
			hierarchy += this.parent.getHierarchy(idBroadcasted);
		for (SiteItf child : this.children)
			hierarchy += child.getHierarchy(idBroadcasted);
